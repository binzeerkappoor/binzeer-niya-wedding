(function () {
  ...
  function openSite() {
    ...
    // Stage 1: the invitation "opens" — seal shrinks, cover fades,
    // names reveal in gold, gold dust bursts from the seal.
    if (openingCard) openingCard.classList.add("is-open");
    spawnOpeningBurst();
    playMusic();

    var revealHoldMs = reduceMotion ? 0 : 1500;

    // Stage 2: after the reveal has been visible for a moment, the
    // burgundy curtains part and the card fades/zooms into the site.
